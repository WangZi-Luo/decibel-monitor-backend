...
# 终极解决方案：中间件方式，在Flask解析之前处理
@app.before_request
def handle_empty_json_body():
    # 如果是application/json但body为空，强制修改Content-Type
    if request.content_type == 'application/json' and (not request.data or len(request.data.strip()) == 0):
        # 清空Content-Type，让Flask不尝试解析JSON
